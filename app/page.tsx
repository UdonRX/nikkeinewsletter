...
type ReaderData = { title: string; imageUrl: string; contentHtml: string; url: string; available: boolean; };
...
export default function Home() {
  ...
  async function openReader(emailIndex: number, newsIndex: number) {
    ...
    setFontScale(1);
    if (!news.url) { setReaderLoading(false); return; }
    try {
      const r = await fetch("/api/article?url=" + encodeURIComponent(news.url));
  ...
  if (error) return <main className="app-shell"><div className="empty card"><h1>日経ニュースメール</h1><p>{error}</p><a className="btn" href="/api/auth/google">Googleで接続</a></div></main>;
  ...
  return (
    <main className="app-shell">
      ...
      {!emails.length ? (
        <div className="empty card"><h2>ニュースメールがありません</h2><p>nikkei-news@mx.nikkei.com または sokuho-news@mx.nikkei.com のメールを待っています。</p></div>
      ) : (
        <div className="mail-stack">
          {emails.map((email, emailIndex) => (
            <section className={"mail-card " + (email.kind === "速報" ? "breaking" : "regular")} key={email.id}>
              <header className="mail-header">
                <div className="mail-type"><span className="type-dot" /><strong>{email.kind}</strong><span className="mail-date">{formatDate(email.receivedAt)}</span></div>
                <span className="mail-count">{email.newsCount}件</span>
              </header>
              ...
              <div className="bento-viewport"><div className="bento-track">
                {email.news.map((news, newsIndex) => (
                  <button type="button" className="bento-news" key={news.url || news.title + newsIndex} onClick={() => openReader(emailIndex, newsIndex)}>
                    <div className="bento-image">{news.imageUrl ? <img src={news.imageUrl} alt="" loading="lazy" /> : <div className="image-placeholder"><span>N</span></div>}</div>
                    ...
                    <div className="bento-open">読む ›</div>
                  </button>
                ))}
              </div></div>
      ...
      {reader && <div className="reader-overlay" role="dialog" aria-modal="true">
        ...
        <article className="safari-reader">
          ...
          {readerLoading ? <div className="reader-loading">記事を読み込んでいます…</div> : reader.data?.contentHtml ? <div className="article-content" style={{ fontSize: fontScale + "em" }} dangerouslySetInnerHTML={{ __html: reader.data.contentHtml }} /> : <div className="reader-unavailable"><p>このニュース本文をリーダー形式で取得できませんでした。</p><p>日経電子版では元の記事を開けます。</p></div>}
